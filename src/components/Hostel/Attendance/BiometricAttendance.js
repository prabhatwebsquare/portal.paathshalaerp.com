...
import { Badge, Box, Button, Flex, IconButton, Input, Select, Table, TableContainer, Tbody, Td, Text, Th, Thead, Tooltip, Tr } from "@chakra-ui/react";
...
export const BiometricAttendance = ({ sessionMasterId, themeColor }) => {
    ...
    const [inputValue, setInputValue] = useState({ fromDate: dayjs().format("YYYY-MM-DD"), endDate: dayjs().format("YYYY-MM-DD") })
    ...
    const { getBioAttendanceAction, getBioAttendanceStatus, bioAttendance, resetBioAttendanceStatus } = useStudentStore(s => ({
        ...
        if (bioAttendance?.length) {
            const data = map(bioAttendance, bio => {
                const dateWise = groupBy(
                    map(bio?.attendances, b => ({ ...b, cDate: dayjs(b.date).format("YYYY-MM-DD") })),
                    "cDate"
                );
                const studentData = {
                    "Roll No.": bio.rollNo || "-",
                ...
                map(datesRange, d => {
                    ...
                    studentData[`${date} In`] = data?.length ? dayjs(data[0]?.date).format("hh:mm A") : "-";
                    studentData[`${date} Out`] = data?.length ? dayjs(data[data?.length - 1]?.date).format("hh:mm A") : "-";
                });
        ...
        <Box>
            <Flex w={"100%"} justify={"space-between"}>
                <form style={{ width: "85%" }} onSubmit={getAttendanceData}>
                    <Flex gap={3}>
                        <CustomInput size={"sm"} w={"20%"} type={"date"} name="fromDate" label={"StartDate"} inputValue={inputValue} setInputValue={setInputValue} />
                        <CustomInput size={"sm"} w={"20%"} type={"date"} name="endDate" label={"EndDate"} inputValue={inputValue} setInputValue={setInputValue} />
                        ...
                        <Button size={"sm"} colorScheme={themeColor} type={"submit"}>Get</Button>
                    </Flex>
                </form>
                <DownloadCSV
                    ...
                    button={<Flex align={"center"}><MdFileDownload /> <Text ml={1}>Excel</Text></Flex>}
                />
            </Flex>
                        ...
                        <Table mt={5}>
                            ...
                            <Tbody>
                                {map(bioAttendance, bio => {
                                    const dateWise = groupBy(map(bio?.attendances, b => ({ ...b, cDate: dayjs(b.date).format("YYYY-MM-DD") })), "cDate")
                                    return (
                                        <Tr>
                                            <Td>{bio.rollNo}</Td>
                                            ...
                                            {map(datesRange, d => {
                                                ...
                                                const atten = data?.[0]
                                                return (
                                                    <Td cursor={"pointer"} onClick={() => setToggleLogs(data)}>
                                                        <Badge variant='outline' colorScheme={
                                                            atten?.isPresent === 1 ? "green"
                                                                : atten?.isPresent === 0 ? "red"
                                                                    : atten?.isPresent === 2 ? "blue"
                                                                        : "yellow"
                                                        }
                                                        >
                                                            {atten?.isPresent === 1 ? "PRESENT"
                                                                : atten?.isPresent === 0 ? "ABSENT"
                                                                    : atten?.isPresent === 2 ? "HALF DAY"
                                                                        : "NOT MARKED"}
                                                        </Badge>
                                                    </Td>
                                                )
            ...
            {toggleDrawer && <MarkMonthlyAttendance data={toggleDrawer} inputValue={inputValue} closeDrawer={() => setToggleDrawer(null)} />}
            {toggleLogs && <BiometricLogs data={toggleLogs} closeModal={() => setToggleLogs(null)} />}
        </Box>
    )
}