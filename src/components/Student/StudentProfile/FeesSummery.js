...
const COLORS = ['#4E91FC', '#FFC542', '#FF7A7A', '#8F00FF'];
const TOTAL_COLOR  = "#A0AEC0"; // Gray for total
const transformData = (data) => {
  return [
    { name: 'Deposite Fees', value: data?.totalFeesCollect || 0, color: COLORS[0] },
    { name: 'Late Fees', value: data?.totalLateFesCollect || 0, color: COLORS[1] },
    { name: 'Discount', value: data?.totalDiscountCollect || 0, color: COLORS[2] },
    { name: 'Due Fees', value: data?.totalDue || 0, color: COLORS[3] },
  ];
};

export const FeesSummery = ({ data }) => {
      ...
      <Flex w={"100%"} mt={3} gap={1} justify={"space-between"} flexWrap="wrap">
                <Flex fontWeight={"semibold"} mb={2}>
                    ...
                    <Box pl={2}>
                        ...
                        <Flex align={"center"} fontSize={13}><MdCurrencyRupee />{sumBy(chartData, "value")}</Flex>
                    </Box>
                </Flex>
                {map(chartData, (d, i) => (
