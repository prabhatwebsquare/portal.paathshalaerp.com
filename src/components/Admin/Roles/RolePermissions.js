import { HasPermitted } from "@/common/HasPermission";
...
export const RolePermissions = ({ data, closeDrawer, themeColor }) => {
  ...
  const { updateRoleAction, updateRoleStatus, resetRoleStatus } = useAdminStore(
    (s) => ({
  ...
  useEffect(() => {
    if (updateRoleStatus === STATUS.SUCCESS) {
      resetRoleStatus();
      closeDrawer();
    }
  }, [closeDrawer, resetRoleStatus, updateRoleStatus]);
      ...
      >
        ...
        <Box pl={8} mt={2}>
          <Flex flexWrap="wrap" gap={4}>
            {map(permission.children[0], (child) => {
              const allSelected = flatMap(permission.children.slice(1)).every((c) =>
        ...
        <Box pl={12} mt={4}>
          ...
          <Flex flexDir="column" gap={2}>
            {map(permission.children.slice(1), (child) => (
              HasPermitted(child?.[0]?.value) && (
                <Flex
                  key={child?.[0]?.value}
                  w="100%"
                  align="center"
                  bg={selected.includes(child?.[0]?.value) ? "blue.50" : "transparent"}
                  p={2}
                  borderRadius="md"
                  _hover={{ bg: "blue.50" }}
                  transition="background 0.2s ease"
                >
                  ...
                  <Checkbox
                    ...
                    colorScheme="blue"
                    isDisabled={!isMenuChecked}
                    isChecked={selected.includes(child?.[0]?.value)}
                    value={child?.[0]?.value}
                  ...
                  {[1, 2, 3].map((i) =>
                    child?.[i] ? (
                      <Checkbox
                        ...
                        colorScheme="blue"
                        isDisabled={!isMenuChecked}
                        isChecked={selected.includes(child?.[i]?.value)}
                        value={child?.[i]?.value}
                        onChange={(e) => handleSelect(e.target.value)}
                      />
                    ) : (
        ...
        <DrawerFooter>
          ...
          <Button
            ...
            isLoading={updateRoleStatus === STATUS.FETCHING}
