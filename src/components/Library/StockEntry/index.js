...
export const StockEntry = ({ themeColor, sessionMasterId }) => {
    ...
    useEffect(() => {
        if ((getPurchaseEntryStatus || 1) === STATUS.NOT_STARTED) {
            getPurchaseEntryAction({ page: 1, limit: 10 })
        }
            ...
            <Box p={5} bg={"white"} h={"75vh"}>
                <Box className="scrollBar" h={"100%"} maxH={"100%"} overflowY={"scroll"}>
                    <Flex mt={2} justify={"space-between"}>
                        <Flex w={"45%"}>
                            <CustomInput autoFocus={true} size={"sm"} type={"text"} name="search" label={"Search By Name/Author"} inputValue={inputValue} setInputValue={setInputValue} />
                        ...
                        {allPurchaseEntrys?.data?.length ?
                            <TableContainer mt={2}>
                                <Table w="100%" size={"sm"} variant={"simple"}>
                                    ...
                                    <Tbody>
                                        {map(allPurchaseEntrys?.data, (entry, index) => (
                            ...
                            :
                            <NoData title={"No Purchase Entry Found"} />
                        }
                    </LoadingContainer>
                </Box>
