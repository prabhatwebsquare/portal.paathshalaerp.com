...
import { AddFeesHead } from "./AddFees";
import { useTransportStore } from "@/store/Transport";

export const FeesHead = ({ themeColor, sessionMasterId }) => {
  const [toggleDrawer, setToggleDrawer] = useState(null);
  const {
    getFeesHeadAction,
    getFeesHeadStatus,
    allFeesHeads,
    deleteFeesHeadAction,
    deleteFeesHeadStatus,
    resetFeesHeadStatus,
  } = useTransportStore((s) => ({
    getFeesHeadAction: s.getFeesHeadAction,
    getFeesHeadStatus: s.getFeesHeadStatus,
    allFeesHeads: s.allFeesHeads,
    deleteFeesHeadAction: s.deleteFeesHeadAction,
    deleteFeesHeadStatus: s.deleteFeesHeadStatus,
  ...
  }, [getFeesHeadAction, getFeesHeadStatus]);

  const deleteFeesHead = (id) => {
    deleteFeesHeadAction(id);
  };

  return (
    <Box pos={"relative"} h={"70vh"}>
      <Box className="scrollBar" maxH={"100%"} overflowY={"scroll"}>
        <LoadingContainer status={getFeesHeadStatus}>
          {allFeesHeads?.length ? (
               ...
               <Thead>
                 <Tr>
                   ...
                   {(HasPermission(PERMISSIONS.TRANS_FEES_DEFINE_EDIT) ||
                     HasPermission(PERMISSIONS.TRANS_FEES_HEAD_DELETE)) && (
                     <Th textAlign="center" w="20%">Action</Th>
                   )}
                 </Tr>
               </Thead>
               <Tbody>
                 {map(allFeesHeads, (fee, index) => (
                   <Tr key={fee.id}>
                       ...
                       <Td textAlign="center">
                         {fee.id !== 1 && (
                           <Flex justify="center" gap={3}>
                             {HasPermission(PERMISSIONS.TRANS_FEES_HEAD_EDIT) && (
                               <Tooltip label="Edit">
                                 <IconButton
                                   size="xs"
                                   icon={<EditIcon />}
                             ...
                             {HasPermission(PERMISSIONS.TRANS_FEES_HEAD_DELETE) && (
                               <DeleteButton
                                 ...
                                 confirm={() => deleteFeesHead(feesHead.id)}
                                 status={deleteFeesHeadStatus}
                                 reset={resetFeesHeadStatus}
      ...
      {HasPermission(PERMISSIONS.TRANS_FEES_HEAD_ADD) && (
