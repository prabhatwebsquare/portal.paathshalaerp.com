...
export const DeleteFees = ({ themeColor, sessionMasterId }) => {
  ...
  const {
    ...
    deleteFeesAction: s.deleteFeesAction,
    deleteFeesStatus: s.deleteFeesStatus,
  }));
  ...
  const getPendingFees = (e) => {
    ...
    const transportSearch = {
      ...
      ...(isAll ? {} : { transportFeeMasterId: [Number(inputValue.feesNameMasterId)] }),
      feesTypeMasterId: 2,
    ...
    const academicSearch = {
      ...
      sessionMasterId,
      ...inputValue,
      ...(isAll ? {} : { feesNameMasterId: [Number(inputValue.feesNameMasterId)] }),
      feesTypeMasterId: 1,
      page: currentPage,
      limit: parseInt(limit),
    };
  ...
  const deleteFees = () => {
    const payload = {
      ...
      ...(inputValue.feesType === "transportFees" 
        ? { transportFeeMasterId: inputValue.feesNameMasterId }
        : { feesNameMasterId: inputValue.feesNameMasterId })
    };
    deleteFeesAction(payload);
  };
      ...
      <Box p={5} bg={"white"} h={"90%"}>
        <Box className="scrollBar" maxH={"100%"} overflowY={"scroll"}>
          <Flex justify={"space-between"}>
            ...
            {pendingFees?.studentData?.length > 0 && selectedStudent?.length > 0 && (
                <DeleteButton
              isButton={true}
              description={"Are you sure? Do you want to delete?"}
              confirm={deleteFees}
              status={deleteFeesStatus}
