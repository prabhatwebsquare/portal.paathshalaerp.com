...
import { StudentRegistration } from "../Transport/TransportStudent/StudentRegistration";
import { CustomSelect } from "@/common/CustomSelect";

export const FeesCollection = ({ themeColor, sessionMasterId }) => {
  ...
  }, [studentFees?.studentFees, updatedDeposits, updatedDepositsLateFee]);
  ...
  useEffect(() => {
    if (getFeesReceiptStatus === STATUS.SUCCESS) {
      ...
      onClose();
      setUpdatedDeposits({});
      setUpdatedDepositsLateFee({});
    }
  }, [feeReceiptData, getFeesReceiptStatus, resetSearch, resetStudentFee]);
  ...
  const showLateFeesColumn = studentFeesDetails.some((fee) => fee.lateFees > 0);
  const showLateFeesColumnTrasport = transportFeesDetails.some(
    (fee) => fee.lateFees > 0
  );
  ...
  const collectFees = async (e) => {
    ...
    const regularFees = await studentFeesDetails
      .filter(
        (data) =>
    ...
    await feesPaidData.push(...regularFees, ...transportFees);

    const feesType =
      inputFeesValue?.feesType === "transport-fees"
    ...
    const temp = await {
      ...
      totalDiscount: sumBy(studentFeesDetails, "discount") || 0,
  ...
  const options = useMemo(() => {
    return studentFeesDetails.map((fee) => ({
      value: fee?.fees_name_master?.name,
      label: fee?.fees_name_master?.name,
    }));
  }, [studentFeesDetails]);
  ...
  // Filter data based on selected fee heads
  const filteredFeesDetails = selectedFeesHead.length
    ? studentFeesDetails.filter((fee) =>
      selectedFeesHead.includes(fee?.fees_name_master?.name)
    )
    : studentFeesDetails;
              ...
              <LoadingContainer status={getStudentFeesStatus}>
                {studentFees ? (
                  <Box>
                    ...
                    >
                      <Grid
                        templateColumns="1.8fr 1fr 2.5fr" // Three columns for the first three cards
                        gap={4}
                        p={5}
                        w="100%"
                        alignItems="stretch"
                      >
                        ...
                        >
                          <Flex direction="row" spacing={2} w="100%">
                            <Box flex="0.8">
                          ...
                          >
                            <Grid templateColumns="repeat(3, 1fr)" gap={4}>
                                ...
                                <Flex direction="column" spacing={0}>
                                  <Flex align="center" mb={1}>
                                    ...
                                    <Text
                                      ...
                                      fontWeight="bold"
                                    >
                                      {studentFees.feesInfo.totalFeesCollect ||
                                        0}
                                    </Text>
                                  </Flex>
                                  <Flex align="center" mb={1}>
                                  ...
                                  >
                                    {studentFees.feesInfo
                                      .totalDiscountCollect || 0}
                                  </Text>
                                </Flex> */}
                                </Flex>
                              </Box>

                              {/* Right Section: Due Fees */}
                              <Box>
                              ...
                              <Button
                                ...
                                variant="solid"
                                borderadius="full" // No rounded corners on the left
                              >
                                View Siblings
                              </Button>
                            </Grid>
                        ...
                        >
                          <TabList mb={2}>
                            <Tab
                              ...
                              _hover={{ bg: "gray.200" }} // Light hover effect
                              bg="gray.100" // Light background for unselected
                            >
                              Academic Fees
                            </Tab>
                          ...
                          <TabPanels>
                              ...
                              {studentFeesDetails ? (
                                <Box mt={0} fontSize={14} p={0}>
                                  <Flex
                                    ...
                                    borderRadius="2xl"
                                  >
                                    <Text>Academic Fees Details</Text>
                                    <Text fontSize={14} fontWeight={"bold"}>
                                      Academic Total Due Fees: &nbsp;{" "}
                                              ...
                                              >
                                                ...
                                                >
                                                  ...
                                                  <Input
                                                    type="number"
                                                    value={
                                                      updatedDeposits[fee.id] ||
                                                      ""
                                                    }
                                                    disabled={
                                                      Number(fee.calDueFees) ===
                                                      0
                                                    }
                                                    fontWeight={700}
                                                    onChange={(e) => {
                            ...
                            <TabPanel p={0}>
                              {filteredFeesDetailsTrasport?.length > 0 ? (
                                <Box mt={0} fontSize={14} p={0}>
                                  <Flex
                                    px={5}
                                    py={1}
                                ...
                                // >
                                //   <Image
                                //     src="https://content.jdmagicbox.com/v2/comp/delhi/y8/011pxx11.xx11.180312181148.e2y8/catalogue/burni-travels-okhla-delhi-van-services-for-schools-0gk5q5236e.jpg"
                                //     alt="No Transport Fees"
                                //     h="200px"
                                //     mx="auto"
                                //     mb={4}
                                //   />
                                ...
                                //     size="lg"
                                //     leftIcon={<FiPlus />}
                                //     onClick={() => setToggleDrawer(studentFees)}
                                //     borderRadius="full"
