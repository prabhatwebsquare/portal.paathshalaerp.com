import { AddIcon, EditIcon } from "@chakra-ui/icons";
import { Box, Flex, IconButton, Table, TableContainer, Tbody, Td, Th, Thead, Tooltip, Tr } from "@chakra-ui/react";
import { map } from "lodash";
import React, { useEffect, useState } from "react";
import { useClassSetupStore } from "@/store/classSetup";
import { STATUS } from "@/constant";
import { DeleteButton } from "@/common/DeleteButton";
import { AddHouse } from "./AddHouse";
import { LoadingContainer } from "@/common/LoadingContainer";
import { useAdditionalSetupStore } from "@/store/additionalSetup";
import { NoData } from "@/common/NoData";
import { HasPermission } from "@/common/HasPermission";
import { PERMISSIONS } from "@/constant/PermissionConstant";

export const House = ({ themeColor }) => {
    const [toggleDrawer, setToggleDrawer] = useState(null)
    const { getHouseAction, getHouseStatus, allHouses, deleteHouseAction, deleteHouseStatus, resetHouseStatus } = useAdditionalSetupStore(s => ({
        getHouseAction: s.getHouseAction,
        getHouseStatus: s.getHouseStatus,
        allHouses: s.allHouses,
        deleteHouseAction: s.deleteHouseAction,
        deleteHouseStatus: s.deleteHouseStatus,
        resetHouseStatus: s.resetHouseStatus
    }))

    useEffect(() => {
        if ((getHouseStatus || 1) === STATUS.NOT_STARTED) {
            getHouseAction()
        }
    }, [getHouseAction, getHouseStatus])

    const deleteHouse = (id) => {
        deleteHouseAction(id)
    }

    return (
        <Box pos={"relative"} h={"70vh"}>
            <Box className="scrollBar" maxH={"100%"} overflowY={"scroll"}>
                <LoadingContainer status={getHouseStatus}>
                    {allHouses?.length ?
                       <Box p={4} bg="white" borderRadius="lg" boxShadow="md" borderWidth="1px" borderColor="gray.300">
                       <TableContainer mt={2}>
                         <Table w="100%" size="md" variant="simple" colorScheme="blue">
                           <Thead>
                             <Tr>
                               <Th textAlign="center" w="80px">S.No.</Th>
                               <Th textAlign="center">House</Th>
                               {(HasPermission(PERMISSIONS.HOUSE_EDIT) || HasPermission(PERMISSIONS.HOUSE_DELETE)) && (
                                 <Th textAlign="center" w="20%">Action</Th>
                               )}
                             </Tr>
                           </Thead>
                           <Tbody>
                             {map(allHouses, (house, index) => (
                               <Tr key={house.id}>
                                 <Td textAlign="center">{index + 1}</Td>
                                 <Td textAlign="center">{house.name}</Td>
                                 {(HasPermission(PERMISSIONS.HOUSE_EDIT) || HasPermission(PERMISSIONS.HOUSE_DELETE)) && (
                                   <Td textAlign="center">
                                     <Flex justify="center" gap={3}>
                                       {HasPermission(PERMISSIONS.HOUSE_EDIT) && (
                                         <Tooltip label="Edit">
                                           <IconButton size="xs" icon={<EditIcon />} colorScheme="blue" onClick={() => setToggleDrawer(house)} />
                                         </Tooltip>
                                       )}
                                       {HasPermission(PERMISSIONS.HOUSE_DELETE) && (
                                         <DeleteButton
                                           description="Are you sure? Do you want to delete?"
                                           confirm={() => deleteHouse(house.id)}
                                           status={deleteHouseStatus}
                                           reset={resetHouseStatus}
                                           buttonProps={{ size: "sm", colorScheme: "red" }}
                                         />
                                       )}
                                     </Flex>
                                   </Td>
                                 )}
                               </Tr>
                             ))}
                           </Tbody>
                         </Table>
                       </TableContainer>
                     </Box>
                     
                        :
                        <NoData title={"No Hounse Found"} />
                    }
                </LoadingContainer>
            </Box>
            {HasPermission(PERMISSIONS.HOUSE_ADD) &&
                <Tooltip placement="top" label={"Add New House"}>
                    <Flex pos={"absolute"} bottom={10} right={10}
                        w={"50px"} h={"50px"}
                        bg={`${themeColor}.500`}
                        justify={"center"}
                        align={"center"}
                        borderRadius={"50%"}
                        color={"white"}
                        onClick={() => setToggleDrawer([])}
                    >
                        <AddIcon />
                    </Flex>
                </Tooltip>
            }
            {toggleDrawer && <AddHouse data={toggleDrawer} themeColor={themeColor} closeDrawer={() => setToggleDrawer(null)} />}
        </Box>
    )
}